...
#ifdef __GPIO_SIMULATE
	snprintf(buffer, GPIO_BUFFER_SIZE, "/supla_virtual_dev/gpio/gpio%i/%s", port_number, file);
#else
	snprintf(buffer, GPIO_BUFFER_SIZE, "/sys/class/gpio/gpio%i/%s", port_number, file);
#endif
	return buffer;
}
...
#ifdef __GPIO_SIMULATE
			if ( port[a].ifd != -1 )
				inotify_rm_watch(inot_fd, port[a].ifd);
#endif
...
char gpio_set_params(char *buffer, const char port_number, const char *dir_val, const char *edge_val) {
	...
	if ( 0 == gpio_read_file(gpio_get_file(buffer, port_number, dir_str), rbuff, 5) )
		return 0;

	if ( strcmp(rbuff, dir_val) != 0 )
		return 0;

	if ( 0 == gpio_read_file(gpio_get_file(buffer, port_number, edge_str), rbuff, 5) )
		return 0;

	if ( strcmp(rbuff, edge_val) != 0 )
		return 0;

...
void ICACHE_FLASH_ATTR supla_esp_http_send_response(struct espconn *pespconn, const char *code, const char *html) {
...
void ICACHE_FLASH_ATTR supla_esp_http_ok(struct espconn *pespconn, const char *html) {
...
void ICACHE_FLASH_ATTR supla_esp_parse_request(TrivialHttpParserVars *pVars, char *pdata, unsigned short len, SuplaEspCfg *cfg) {
...
void ICACHE_FLASH_ATTR supla_esp_recv_callback (void *arg, char *pdata, unsigned short len) {
...
void ICACHE_FLASH_ATTR supla_esp_cfgmode_start(void) {
	...
	apconfig.ssid_len = strlen(APSSID);
	apconfig.channel = 1;

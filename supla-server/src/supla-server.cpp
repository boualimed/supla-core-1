...
int main(int argc, char* argv[]) {
	...
	if ( scfg_bool(CFG_SSL_ENABLED) == 1 ) {
		...
		if ( 0 == ( ssd_ssl = ssocket_server_init(scfg_string(CFG_SSL_CERT), scfg_string(CFG_SSL_KEY), scfg_int(CFG_SSL_PORT), 1) )
	...
	if ( scfg_bool(CFG_TCP_ENABLED) == 1 ) {
		...
		if ( 0 == ( ssd_tcp = ssocket_server_init("", "", scfg_int(CFG_TCP_PORT), 0) )
